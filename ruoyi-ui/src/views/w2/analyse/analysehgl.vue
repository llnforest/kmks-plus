<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="recordsList" @selection-change="handleSelectionChange" @row-click="rowClick" @row-contextmenu="rightClick">
      ...
      <el-table-column label="考试人次" align="center" prop="peopleTimes" min-width="80"/>
      <el-table-column label="考试人数" align="center" prop="peopleNum" min-width="80"/>
      <el-table-column label="合格人次" align="center" prop="passTimes" min-width="80"/>
      <el-table-column label="合格人数" align="center" prop="passNum" min-width="80"/>
      <el-table-column label="人次合格率" align="center" prop="passTimesRate" min-width="80"/>
      <el-table-column label="人数合格率" align="center" prop="passNumRate" min-width="80"/>
      <el-table-column label="一次合格人数" align="center" prop="passFir" min-width="110"/>
      <el-table-column label="一次合格率" align="center" prop="passFirRate" min-width="110"/>
      <el-table-column label="二次合格人数" align="center" prop="passSec" min-width="110"/>
      <el-table-column label="二次合格率" align="center" prop="passSecRate" min-width="110"/>
    </el-table>
...
import {getAnalyseHgl} from "@/api/w2/records";
...
export default {
  ...
  data() {
    return {
      ...
      openReset: false,
      // 驾校名称时间范围
      daterangeKsrq: getTodayDateRange(),
      // 查询参数
      queryParams: {
        pageNum: 1,
        pageSize: 10,
  ...
  created() {
    this.schoolList = this.getSchoolList();
  ...
  methods: {
    ...
    getList() {
      ...
      if (null != this.daterangeKsrq && '' != this.daterangeKsrq) {
        this.queryParams.params["beginKsrq1"] = this.daterangeKsrq[0];
        this.queryParams.params["endKsrq1"] = this.daterangeKsrq[1];
      }
      getAnalyseHgl(this.queryParams).then(response => {
        this.recordsList = response.data;
        this.recordsList.forEach(record =>{
          if(record.jxdm == 0){
            record.jxmc = this.schoolList[0].paramname;
          }else{
            record.jxmc = this.schoolMap[record.jxdm]
          }
        })
    ...
    getSchoolList() {
      selectSchool({ type: 3 }).then(response => {
        ...
        this.schoolList.forEach(v=>this.schoolMap[v.paramdm] = v.paramname)
        this.getList();
      });
    },
    // 取消按钮
