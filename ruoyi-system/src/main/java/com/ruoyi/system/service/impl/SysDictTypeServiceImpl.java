...
public class SysDictTypeServiceImpl implements ISysDictTypeService, DictService {
    ...
    public String getDictLabel(String dictType, String dictValue, String separator) {
        // 优先从本地缓存获取
        List<SysDictData> datas = null;
        boolean requestStatus = true;
        try{
            datas = (List<SysDictData>) SaHolder.getStorage().get(CacheConstants.SYS_DICT_KEY + dictType);
        }catch (NotWebContextException ex){
            requestStatus = false;
        }
        if (ObjectUtil.isNull(datas)) {
            datas = SpringUtils.getAopProxy(this).selectDictDataByType(dictType);
            if(requestStatus) SaHolder.getStorage().set(CacheConstants.SYS_DICT_KEY + dictType, datas);
        }
