...
public class CarController extends BaseController {
     ...
     * 上传GPS轨迹
     *
     * @param carMapDto carMapDto
     * @return {@link List}<{@link CarSignalDto}>
     */
    @PostMapping("/gps")
    @Async
    public CompletableFuture<R<Void>> gps(@Validated @RequestBody CarMapDto carMapDto) {
        if (gpscontentService.insertRawSql(carMapDto.getSql())) {
            return CompletableFuture.completedFuture(R.ok("success"));
        } else {
            return CompletableFuture.completedFuture(R.fail("fail"));
        }
        // 获取车辆信号并转数据结构
    ...
    public R<List<CarSignalDto>> carSignal(@RequestParam(name = "kch") @NotBlank(message = "考车号不能为空") String kch) {
        // 获取车辆信号并转数据结构
...
//        }
    ...
    public R<List<CenterConfigDto>> centerConfig(@RequestParam(name = "kch") @NotBlank(message = "考车号不能为空") String kch) {
        // 获取中心配置并转数据结构
    ...
    public R<List<DeductPointsDto>> centerKf(@RequestParam(name = "kch") @NotBlank(message = "考车号不能为空") String kch) {
        // 获取中心配置并转数据结构
        W2KfconfigBo w2KfconfigBo = new W2KfconfigBo();
        HashMap<String, Object> map = new HashMap<>();
        map.put("judgeType", "2");
        ...
        List<DeductPointsDto> deductPointsDtos = vos.stream().map(vo -> new DeductPointsDto(vo.getValue(), vo.getGakfdm(), vo.getGakfmc(), String.valueOf(vo.getKf()), vo.getAutoflag())).collect(Collectors.toList());
    ...
    public R<List<DeductPointsDto>> deductPoints(@RequestParam(name = "kch") @NotBlank(message = "考车号不能为空") String kch) {
        //获取系统考试科目
    ...
    public R<List<CarModelDto>> carModel(@RequestParam(name = "kch", required = true) @NotBlank(message = "考车号不能为空") String kch) {
        // 获取考车信息
        W2KcxxBo w2KcxxBo = new W2KcxxBo();
        w2KcxxBo.setKch(kch);
    ...
    public R<List<FieldMapDto>> fieldMap(@RequestParam(name = "kch") @NotBlank(message = "考车号不能为空") String kch) {
        // 获取考车信息
        W2KcxxBo w2KcxxBo = new W2KcxxBo();
        w2KcxxBo.setKch(kch);
        W2KcxxVo w2KcxxVo = kcxxService.queryOne(w2KcxxBo);
    ...
    public R<CarInfoDto> carInfo(@RequestParam(name = "kch") @NotBlank(message = "考车号不能为空") String kch) {
        isCanVisited(kch);
        // 获取考车信息数据并转数据结构
        W2KcxxBo w2KcxxBo = new W2KcxxBo();
        w2KcxxBo.setKch(kch);
        ...
        //增加额外项目拼接项目序号
        String additional = configService.selectConfigByKey(CacheNames.PROJECT_IDS_EXPECT_KEY) + ",";
        String xmxh = "";
        if (carInfoDto.getXmxh().contains(";")) {
            String[] xmxhs = carInfoDto.getXmxh().split(";");
            xmxh = additional + xmxhs[0] + ";" + additional + xmxhs[1];
        } else {
            xmxh = additional + carInfoDto.getXmxh();
        }
        carInfoDto.setXmxh(xmxh);
